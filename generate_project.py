...
class ProjectConfigError(ValueError):
    """Raised when the incoming project configuration is invalid."""


def xml_escape(text: str) -> str:
    """Escaped tekst zodat die veilig in een Android XML resource-waarde past."""
    return (text.replace("&", "&amp;")
                .replace("<", "&lt;")
                .replace(">", "&gt;")
                .replace('"', "&quot;")
...
def kt_escape(text: str) -> str:
    """Escaped tekst zodat die veilig in een Kotlin/Java dubbele-quote string past."""
    return text.replace("\\", "\\\\").replace('"', '\\"')


def safe_resource_name(app_name: str) -> str:
    """Zet een vrije app-naam om naar een geldige Android resource-/stijlnaam
    (alleen [a-zA-Z0-9_], mag niet met een cijfer beginnen, nooit leeg)."""
    cleaned = re.sub(r'[^a-zA-Z0-9_]', '', app_name.replace(" ", ""))
    if not cleaned or cleaned[0].isdigit():
        cleaned = f"App{cleaned}"
    return cleaned


def safe_folder_name(app_name: str) -> str:
    """Zet een vrije app-naam om naar een veilige map-/bestandsnaam.
    Verwijdert padscheidingstekens en '..' om path traversal te voorkomen
    (bv. een app-naam "../../evil" mag nooit buiten de output-map schrijven)."""
    name = app_name.strip().replace(" ", "-").lower()
...
def validate_config(config: dict):
    ...
    if lang not in ("kotlin", "java"):
        raise ProjectConfigError("lang moet 'kotlin' of 'java' zijn")

    arch = config.get("arch", "mvvm")
    if arch not in ("mvvm", "mvc", "clean", "none"):
        raise ProjectConfigError("arch moet mvvm, mvc, clean of none zijn")

    valid_features = {"internet", "camera", "location", "storage", "push",
                       "bluetooth", "biometric", "firebase"}
    features = config.get("features", ["internet"])
    if not isinstance(features, list) or not all(f in valid_features for f in features):
        raise ProjectConfigError(f"features moet een lijst zijn uit: {sorted(valid_features)}")
...
def create_project(config: dict, output_dir: str = ".", make_zip: bool = True):
    ...
    app_id = config.get("app_id", "com.example.app").strip()
    min_sdk = int(config.get("min_sdk", DEFAULT_MIN_SDK))
    target_sdk = int(config.get("target_sdk", DEFAULT_TARGET_SDK))
    lang = config.get("lang", "kotlin")
    arch = config.get("arch", "mvvm")
    ...
    project_dir = (Path(output_dir) / folder_name).resolve()
    output_root = Path(output_dir).resolve()
    # Defensieve check: project_dir moet altijd binnen output_dir blijven.
    if output_root not in project_dir.parents and project_dir != output_root:
        raise ProjectConfigError("Ongeldige projectmap (buiten output directory).")
    ...
    print_instructions(folder_name, lang)
...
class MainViewModel : ViewModel() {{
    ...
    init {{ _message.value = "Welkom bij {kt_escape(app_name)}!" }}
...
'''
    ...
    <string name="welcome_message">Welkom bij {xml_escape(app_name)}!</string>
...
def print_instructions(folder_name, lang):
    print("\n📋 INSTALLATIE-INSTRUCTIES")
    print("=" * 40)
    print(f"1. Open Android Studio")
    print(f"2. File → Open → selecteer map: {folder_name}/")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", "-n", help="App naam (bijv. 'Mijn App') — verplicht tenzij --config gebruikt wordt")
    parser.add_argument("--id", "-i", help="App package ID (bijv. com.mijn.app) — verplicht tenzij --config gebruikt wordt")
    ...
    if args.config:
        ...
        if not args.name or not args.id:
            parser.error("--name en --id zijn verplicht, tenzij --config wordt gebruikt")
        config = {
            "app_name": args.name,
            "app_id": args.id,
            "lang": args.lang,
